...
async fn simple_read_parquet_test() -> datafusion::error::Result<()> {
    // register the table
    let ctx = SessionContext::new();
    ctx.register_parquet("example", "tests/data/alltypes_plain.parquet", ParquetReadOptions::default()).await?;
    ...
    // create a plan to run a SQL query
    let df = ctx.sql("SELECT id <= 5, sum(double_col) FROM example group by id <= 5").await?;
