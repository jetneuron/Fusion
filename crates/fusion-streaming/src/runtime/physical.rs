...
impl PhysicalTask {
    ...
    pub fn new_with_watermark(logical: Box<dyn LogicalTask + Send>, watermark: Watermark, execution_context: Arc<Mutex<GraphContext>>) -> PhysicalTask {
        let s = SystemTime::now();
        let time = s.duration_since(SystemTime::UNIX_EPOCH).expect("Time went backwards").as_micros();
        PhysicalTask {
    ...
    pub async fn init_script_env(&self) {
        ...
        globals.set(scope_name, this_table).expect("failed to set global table");
    }
    ...
    /// * `row` - data row
    /// * `ctx` - Context
    pub async fn compute(&self, row: Row, ctx: Context) {
        let logical_task = &self.logical.lock().await;
    ...
    pub fn link(&self, target: &Arc<Mutex<Self>>, edge_condition: Option<EdgeCondition>) -> Vec<JoinHandle<()>>
    {
        ...
        vec![target_process_input_handle, current_process_internal_channel]
    }
    ...
    pub fn launch(&self) -> JoinHandle<()> {
        ...
        tokio::task::spawn(async move {
            ...
            let context_ptr = Box::into_raw(Box::new(ctx));
            match self_logical.internal_launch(context_ptr).await {
                Ok(_) => {}
                Err(error) => {
                    eprintln!("{}", error);
                }
    ...
    fn current_handle_internal_sent(&self, target: &Arc<Mutex<Self>>) -> JoinHandle<()> {
                ...
                match receive_from_target.recv().await {
                    Ok(row) => {
                        if row.is_watermark() {
                            ...
                            debug!("feedback receive eof {}", &row.source);
                            break;
                        }
                    }
    ...
    fn target_handle_current_sent(&self, target: &Arc<Mutex<PhysicalTask>>, edge_condition: Option<EdgeCondition>) -> JoinHandle<()> {
        let self_cloned = self.core.clone();
        ...
        tokio::task::spawn(async move {
            let mut receiver = {
                ...
                (self_tc.get_unit_id(), self_tc.channel.subscribe(), lua_scope_context_name)
            };
            ...
            let ch = {
                ...
                (target_unit, target_core.channel.internal_channel.0.clone(), internal_sender.clone(), watermark.clone())
            };
                    ...
                    Ok(mut row) => {
                        if !started {
                            ...
                            if target_physical_task.stats.start_time_millis == 0 {
                                ...
                                target_physical_task.on_start(Row::start(), &context).await;
                            }
                            started = true;
                        }
                        ...
                        if row.is_eof() {
                            ...
                            if finished {
                                let cloned = row.clone();
                                {
                                    ...
                                    target_physical_task.on_eof(row, &context).await;
                                }
                            ...
                            if filter_fn.is_none() {
                                let target_physical_task = target_cloned.lock().await;
                                target_physical_task.compute(row, context.clone()).await;
                            } else {
                            ...
                            internal_sender.send(Row::watermark(self_id.clone(), recv_offset)).unwrap();
                        }
                    }
                    Err(broadcast::error::RecvError::Lagged(count)) => {
                        println!("消费滞后：{}", count);
    ...
    async fn init_filter(edge_condition: Option<EdgeCondition>, execution_context: Arc<Mutex<GraphContext>>) -> Option<Function> {
        if let Some(condition) = edge_condition {
            if let Some(script) = condition.get_script() {
                ...
                let func = lua.load(format!(r#"
                    function(row)
                    {}
                    end
                    "#, script)).eval::<mlua::Function>().expect("");
                Some(func)
            } else {
                None
            }
