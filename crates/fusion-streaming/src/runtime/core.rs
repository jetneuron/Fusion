...
impl PhysicalGraph {
    pub fn new(logical_graph: LogicalGraph, plugin_manager: Arc<Mutex<PluginManager>>, graph_lua: Arc<Mutex<Lua>>) -> Self {
        PhysicalGraph {
            logical_graph,
            plugin_manager,
            graph_lua,
    ...
    pub async fn execute(&self) {
        log::debug!("Prepare transfer logical graph as pet graph");
        ...
        let start_nodes: Vec<NodeIndex> = pet_graph.node_indices()
            .filter(|&node| {
        ...
        for index in indices {
            let outgoing_count = pet_graph.neighbors_directed(index, petgraph::Direction::Outgoing).count();
            let incoming_count = pet_graph.neighbors_directed(index, petgraph::Direction::Incoming).count();

            if let Some(unit) = pet_graph.node_weight(index) {
                let unit_id = unit.get_id();
                let unit_type = unit.get_type();
                log::debug!("Initialize physical, id[{}], type={}, incoming={}, outgoing={}",
                    unit_id, unit_type, incoming_count, outgoing_count);
                ...
                let logical_task = create_result.expect(format!(
                    "Could not create logical task, type = {}, version = {}", r#type, version).as_str());
                ...
                let watermark = {
                    ...
                    if outgoing_count > 0 {
                        ...
                        Watermark::new(u64::MAX, u64::MAX - 100, u64::MAX - 200, incoming_count as i8)
                    }
                };
                let physical: PhysicalTask = PhysicalTask::new_with_watermark(logical_task, watermark, graph_context.clone());
        ...
        for start_node in start_nodes.clone() {
            ...
            while let Some(node) = dfs.next(&pet_graph) {
                ...
                let current_node = physical_map.get(&node)
                ...
                for neighbor in outgoing {
                    ...
                    let edge_index = pet_graph.find_edge(curr_node_id.clone(), neighbor).expect("Could not find edge in graph");
                    let edge = pet_graph.edge_weight(edge_index).expect("Fail to get edge weight");

                    // parse edge condition
                    let edge_condition = edge.get_config().map(|ec| {
                        serde_json::from_value::<EdgeCondition>(ec["condition"].clone()).map(|s| s)
                    }).filter(|t| t.is_ok()).map(|t| t.unwrap());

                    let target_physical = physical_map.get(&neighbor)
        ...
        for start_node in start_nodes {
            let current_node = physical_map.get(&start_node)
                .expect("Could not find node in map");
            let physical_node = current_node.lock().await;

            let unit = pet_graph.node_weight(start_node).expect("Could not find node weight");
            let id = unit.get_id();
            join_handles.push(physical_node.launch());
            log::debug!("Launched physical id[{}]", id);
        }

        let clock = Instant::now();
        futures::future::join_all(join_handles).await;

        log::debug!("Graph had finished in {}ms", clock.elapsed().as_millis());
    }
}
...
impl UserData for LuaRow {
    fn add_methods<M: UserDataMethods<Self>>(methods: &mut M) {
        methods.add_async_meta_function(mlua::MetaMethod::Index, |lua, (row, key): (LuaRow, String)| async move {
            if key.eq("offset") {
            ...
            match index.get(&key) {
                ...
                Some(idx) => {
                    ...
                    match c.dt.unwrap() {
                        ...
                        DataType::str => Ok(Value::String(lua.create_string(c.str_val.as_bytes()).expect("Fail to create string"))),
                        DataType::bool => Ok(Value::Boolean(c.bool_val)),
                        DataType::bytes => unimplemented!("unimplemented type: Bytes"),
                        DataType::json => Ok(Value::String(lua.create_string(c.str_val.as_bytes()).expect("Fail to create string"))),
                    }
                }
            }
        });

        methods.add_async_meta_function(mlua::MetaMethod::NewIndex, |lua, (mut row, key, value): (LuaRow, mlua::String, Value)| async move {
